...
import { addCallLog, editCallLogFull, deleteCallLog, requestAllPermissions, readCallLogs } from '../../lib/CallLogUtil';
...
const Editlog = () => {
    ...
    const logInfo = JSON.parse(params.log as string);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const rowsUpdated = await editCallLogFull({
                ...
                number: phoneNumber.toString(),
                type: callType.toUpperCase(),
                duration: (callType == 'Missed' || callType == 'Rejected') ? '0' : duration.toString(),
                timestamp: Number(selectedDate),
                name: contactName,
                simId: '',
            });

            console.log('Call log edited successfully!', rowsUpdated);
            ToastAndroid.show('Call log edited successfully!', ToastAndroid.SHORT);
            router.push('/')
        } catch (error) {
            ToastAndroid.show('Failed to edit call log!', ToastAndroid.SHORT);
        }
    ...
    useEffect(() => {
        setContactName(logData?.name ?? '')
        setPhoneNumber(logData?.phoneNumber?.toString() ?? '')
        ...
        setSelectedDate(new Date(Number(logData?.timestamp)))


        setDuration(logData?.duration?.toString() ?? '1')
    }, [logData])
    ...
    return (
        ...
        <View className="flex-1 bg-white">
            ...
            >
                ...
                {/* {logData?.name && <View className="mb-5">
